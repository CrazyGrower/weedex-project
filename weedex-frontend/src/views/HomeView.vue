...
const fetchStrains = async () => {
  try {
    ...
    strains.value = response.data.strains.slice().sort((a: Strain, b: Strain) => Number(a.number ?? 0) - Number(b.number ?? 0));
...
const sortStrains = () => {
  strains.value.sort((a: Strain, b: Strain) => Number(a.number ?? 0) - Number(b.number ?? 0));
};
